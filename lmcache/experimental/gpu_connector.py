...
class VLLMPagedMemGPUConnectorMLA(GPUConnectorInterface):
    ...
    Note the memory assigned each token in KV cache allocation is
    aligned to 256 bytes for performance reason. So the actual size 
    for each token is not kv_cache.shap[-1], but the output of
    align_to_256bytes(head_size, kv_dtype), which is saved in
    aligned_head_size.
