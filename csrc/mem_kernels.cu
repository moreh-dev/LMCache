...
__device__ __forceinline__ int64_t key_value_offset(
        const int k_or_v, const int layer_idx, const int token_idx, const int scalar_offset,
...
__global__ void load_and_reshape_multi_layer_kernel(
    scalar_t * __restrict__ key_value,  // [2, num_layer, num_tokens, scalars_per_token]
    scalar_t ** __restrict__ paged_buffer_ptrs, // [num_layers] * [2, PAGE_BUFFER_SIZE, scalars_per_token]
    const int64_t * __restrict__ slot_mapping,  // [num_tokens]
